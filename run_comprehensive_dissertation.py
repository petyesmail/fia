...
def run_scenario(
    ...
    algorithms = {
        'SPR': ShortestPathRouting(config),
        'EAR': EnergyAwareRouting(config),
        'ALB': AdaptiveLoadBalancing(config),
        'DRL-SDN': DRLSDNRouting(config)
    }
    ...
    for run_idx in range(num_runs):
        ...
        for idx, pos in enumerate(positions):
            node = SensorNode(
                node_id=f"N{idx}",
                position=pos,
                initial_energy=run_config.initial_energy,
                comm_range=run_config.comm_range
            )
